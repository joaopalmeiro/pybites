...
class BMI(NamedTuple):
    weight: int
    length: int


def calc_bmi(weight: int, length: int) -> float:
    """Provided/DONE:
...
def create_parser() -> argparse.ArgumentParser:  # https://github.com/python/typeshed/blob/master/stdlib/2and3/argparse.pyi
    """TODO:
...
def handle_args(args: Optional[Union[argparse.Namespace, BMI]] = None) -> None:
    """Provided/DONE:
...
if __name__ == "__main__":
    # handle_args(BMI(weight=80, length=187))
    handle_args()
